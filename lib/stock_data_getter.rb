...
class StockDataGetter
  ...
  def update_price_data(code)
    ...
    if File.exist?(data_file_name)
      get_from_date
      append_to_file(prica_text)
    else
      save_to_file(price_text)
    end
  end
  ...
  def reg_prices
    %r!<td>(\d{4}年\d{1,2}月\d{1,2}日)</td><td>((?:\d|,)+)</td><td>((?:\d|,)+)</td><td>((?:\d|,)+)</td><td>((?:\d|,)+)</td><td>((?:\d|,)+)</td><td>((?:\d|,)+)</td>!
  end
